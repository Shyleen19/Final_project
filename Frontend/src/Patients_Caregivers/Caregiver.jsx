...
const PatientCaregivers = ({ token, caregiver }) => {
    ...
    const handleAddCaregiver = async () => {
        ...
        try {
            ...
            const response = await BackendConnection.add_caregiver({
                'email': email,
    ...
    return (
        <div className="flex h-screen overflow-hidden">
            ...
            <div className="flex-1 bg-[#EDFCFF] p-5 overflow-y-auto">
                    ...
                    <table className="w-full bg-white rounded-lg shadow border-collapse">
                        <thead className="bg-[#00D9FF] text-black">
                            <tr>
                                ...
                                <th className="py-6 px-4 border-b text-left">STATUS</th>
                                    ...
                                    patients.map((patient, index) => (
                                        <tr key={patient.id} className={`hover:bg-gray-100 ${index % 2 === 0 ? 'bg-[#EDFCFF]' : 'bg-white'}`}>
                                            ...
                                            <td className="py-8 px-4 border-b">
                                                <span className={`px-2 py-1 rounded text-sm ${patient.status === 'Accepted' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
                                                    {patient.status}
                                                </span>
                                            </td>
                                ...
                                caregivers.length === 0 ? (
                                    ...
                                    caregivers.map((caregiver, index) => (
                                        <tr key={caregiver.id} className={`hover:bg-gray-100 ${index % 2 === 0 ? 'bg-[#EDFCFF]' : 'bg-white'}`}>
                                            <td className="py-8 px-4 border-b">{caregiver.display_first_name || caregiver.first_name || '-'}</td>
                                            <td className="py-8 px-4 border-b">{caregiver.display_last_name || caregiver.last_name || '-'}</td>
                                            <td className="py-8 px-4 border-b">{caregiver.relationship_with_patient}</td>
                                            <td className="py-8 px-4 border-b">{caregiver.email}</td>
                                            <td className="py-8 px-4 border-b">
                                                <span className={`px-2 py-1 rounded text-sm ${caregiver.status === 'Accepted' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
                                                    {caregiver.status}
                                                </span>
                                            </td>
                                            <td className="py-8 px-4 border-b flex space-x-4">
                        ...
                        <form className="bg-white rounded-2xl shadow-lg w-full max-w-md p-6 relative" onSubmit={handleAddCaregiver}>
                            ...
                            <div className="space-y-6">
                                <div>
                                    ...
                                    <input type="text" name="firstName" value={firstName} onChange={(e) => (setFirstName(e.target.value))} placeholder="Enter First Name" className="w-full border-0 border-b-2 border-gray-300 focus:border-[#00D9FF] focus:ring-0 px-2 py-1" required />
                                </div>

                                <div>
                                    <label className="block text-gray-700 mb-1">Last Name</label>
                                    <input type="text" name="lastName" value={lastName} onChange={(e) => (setLastName(e.target.value))} placeholder="Enter Last Name" className="w-full border-0 border-b-2 border-gray-300 focus:border-[#00D9FF] focus:ring-0 px-2 py-1" required />
                                </div>
